...
module.exports = class Util {
            ...
            return {
                ...
                embeds: [
                    new MessageEmbed()
                        ...
                        .setDescription(
                            `Hey ${member.displayName}, Welcome to the server <a:welcome:1286995862907519056>.`
                        )
                ]
            }
        }
        ...
        try {
            ...
            if (funny === 'first') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:antinuke:1286981353396768780> Antinuke \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'second') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:moderation:1286979686504599563> Moderation \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'third') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:utility:1286979761289035857> Utility \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'fourth') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:welcome:1286979821871824989> Welcomer \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'fifth') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:voice:1286980390459805760> Voice \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'six') {
                ...
                embed.addField(
                    `**<:roles:1250531047595053150> Customrole \`[${cmd.length}]\`**`,
                    cmd.sort().join(', ')
                )
                await interaction
                    .update({
            ...
            if (funny === 'seven') {
                ...
                embed.addField(
                    `**<:automod:1286980167826276382> Automod \`[${cmd.length}]\`**`,
                    cmd.sort().join(', ')
                )
                await interaction
                    .update({
    ...
    async pagination(message, description, desc = '') {
        ...
        let previousbut = new MessageButton()
            ...
            .setEmoji('<:ARROW1:1286996914948280333>')
            .setStyle('SUCCESS')
        let nextbut = new MessageButton()
            .setCustomId('queuenext')
            .setEmoji('<:ARROW:1286996969725759580>')
            .setStyle('SUCCESS')
